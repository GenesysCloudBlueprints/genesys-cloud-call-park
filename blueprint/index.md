...
For more details on Genesys Cloud blueprint support and practices, see Genesys Cloud blueprint [FAQ](https://developer.genesys.cloud/blueprints/faq "Opens the Genesys Cloud blueprint FAQs") sheet.
:::

This Genesys Cloud Developer Blueprint explains how to set up Genesys Cloud to park an active voice call with a code and retrieve it using the code.

When a Genesys Cloud user transfers an active call to an in-queue flow with a code, the call can be retrieved from another phone, station, or by another user using the code associated with the call.
...
:::primary
  **Note:** The example shows a call retrieved from a Google Voice number. A Genesys Cloud user can also retrieve a call from the Genesys Cloud platform. The Genesys Cloud user can send the retrieval code to another Genesys Cloud user via the Genesys Cloud Collaborate Chat or SMS. An example message can be: "You have a parked call. Dial 3172222123 to retrieve the parked call."
  :::
...
Implement the Genesys Cloud objects manually:
...
1. Set the following environment variables in a terminal window before you can run this project using the Terraform provider:
...
Set the following values in the `blueprint/terraform/dev.auto.tfvars` file that are specific to your Genesys Cloud organization:

* `client_id` - The value of your OAuth Client ID using Client Credentials that must be used for the data action integration.
* `client_secret` - The value of your OAuth Client secret using Client Credentials that must be used for the data action integration.
...
To use the blueprint solution in your organization, execute the following commands from the `/terraform` directory: 

* `terraform init` - This command initializes a working directory that contains the Terraform configuration files.
  
* `terraform plan` - This command executes a trial run on your Genesys Cloud organization and displays a list of all the Genesys Cloud resources created. Ensure to review the list before proceeding to the next step.

* `terraform apply --auto-approve` - This command creates and deploys the necessary objects in your Genesys Cloud account. The `--auto-approve` flag completes the required approval step before the command creates the objects. The command output lists all the objects successfully created by Terraform. 
...
* To clean up the org and remove all the objects managed by the local Terraform state file, use the `terraform destroy --auto-approve` command from the `docs/terraform` directory. Ensure that you have backed up your projects before deleting the projects.
...
Create a custom role with the following permissions for use with the Genesys Cloud OAuth client.

| Roles           | Permissions | Role Name |
|-----------------|-------------------------|---------|
| Custom role 1 | **Conversation** > **All Permissions**, **Analytics** > **Conversation Detail** > **All Permissions** and **Analytics** > **Agent Conversation Detail** > **View**   | Orbit Data Actions OAuth Client |
...
3. Search and select the required permissions for each of the custom role.
...
Add a Genesys Cloud data action integration for each OAuth client used in this blueprint to call the Genesys Cloud public API for the following actions:
* Get waiting calls in a specific queue based on External Tag
* Replace participant with the ANI
...
Import each of the data action files and associate with the Orbit Data Actions OAuth Integration data action, which uses the Orbit Data Actions OAuth Client.
...
3. Select the `Get-Waiting-Calls-in-specific-Queue-based-on-External-Tag.custom.json` file and associate with the [Orbit Data Actions OAuth Integration](#add-genesys-cloud-data-action-integrations "Goes to the Add Genesys Cloud data action integrations section") integration, and then click **Import Action**.
   ...
   ![Import the Get Waiting Calls in a specific Queue based on External Tag data action](images/4BImportGetWaitingCallDataAction.png "Import the Update Genesys Cloud User Presence data action")
...
20. Review your workflow. Map the data actions referenced in the flow to the data actions with the corresponding names that you imported in this blueprint solution. For the **Get Waiting Calls in specific Queue based on External Tag** action, make sure that the value in the **holdingQueueId** parameter matches the ID of **InQueue - Orbit Call Park Hold** in-queue call flow. Click **Save** and then click **Publish**.
...
2. Enter a name for the queue.
   ...
   **Note**: Ensure that you add yourself to the queue you have created.

3. Follow the steps 1 and 2 to create another queue for **Call Park - Agent inbound flow** and name the queue as **Call Park Agent Inbound Queue**.

### Import the script

Create a trigger that invokes the created Architect workflow.

1. From **Admin** in Genesys Cloud, search for **Scripts** and navigate to the Scripts list and click **Import**.
...
6. Expand the **Data Actions.Execute Data Action** section. Select the category (or data action integration) for your data action, then select the **Update External Tag on Conversation** data action from the **Data Action** drop menu. Expand the input section and map the script variables to the data action inputs.
   ![Configure the Script](images/ConfigureScript2.png "Configure the Script")

7. Expand the **Scripter.Blind Transfer** section. Select the queue in which you want to park the call. In this example, the Orbit queue is selected.
   ![Configure the Script](images/SelectBlindTransferQueue.png "Configure the Script")

8. Click **Save** and then **Publish** the script.

 
### Add Script to Queue

1. From **Admin** in Genesys Cloud, search for **Queues** and navigate to the Queues list.  Click any inbound queue that you want your agents to be able to park calls from.

2.  From the queue record, click the **Voice** tab and select the **Orbit Queue Transfer** script as the default script for the queue. Click **Save**.
   ![Configure the Script](images/UseScriptInQueue.png "Use the Script")


### Create call route for parked calls

1. Click the **Voice** tab, define a **Calling Party Number** and select the **In-queue Flow** for your queue. Select the **InQueue - Orbit Call Park Hold** flow that you created previously.
   ![Configure Queue](images/VoiceConfigQueue.png "Configure Queue")

2. From **Admin** in Genesys Cloud, navigate to **Routing** > **Call Routing** and click **+Add**.
   ![Create Call Route](images/CreateCallRoute.png "Create Call Route")

3. Enter a name for the call route, an inbound number, and route it to the **Orbit - Parked Call Retrieval** inbound call flow that you created previously.
   ![Configure Call Route](images/ConfigureCallRoute.png "Configure Call Route")

4. Repeat steps 1 to 3, and create another call route. Name it **Call Parking Inbound Agent Flow** and add **Call Park - Agent Inbound Flow** to Route to. 
...
2. Make sure that you set the **holidingQueueId** same as the Id of the **Orbit** queue.
